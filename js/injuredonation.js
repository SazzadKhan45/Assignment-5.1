...
.addEventListener('click', function(){
    // Validation function
    const inputNumber = getInputByIdValue('input-money-value-3');
    if(isNaN(inputNumber)){
        alert('Your input is not a number');
        document.getElementById('input-money-value-3').value = '';
        return;
    }

    // get feni donate blance 
    const textNumber = getTextbyIdValue('injure-donate-balance');
    const newBalance = inputNumber + textNumber;
    document.getElementById('injure-donate-balance').innerText = newBalance;
    // Main balanace 
    ...
    div.innerHTML = `
        <div class = "bg-orange-200 rounded-lg py-5 px-5 text-black my-5">
            ...
            <p>Aid for donation total amount : ${newBalance} Tk</p>
            <p>Aid for donation amount : ${inputNumber} Tk</p>
